...
--Kisiler tablosundan ilk bes veriyi listeleyiniz
select*from kisiler limit 5;

--ilk 2 veriden sonraki 5 veriyi listeleyiniz
...
select ulke, round(avg(maas)) as ortalama_maas --yuvarlama yapmak istersek
...
group by ulke,sehir
--order by ulke  eklersek rastgele sıralamnın onune gecer

-- Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını sorgulayınız.
select*from personel; ----> tabloyu gormek ıcın yazdım
select ulke,cinsiyet, count(sehir) as calisan_sayisi, round(avg(yas)) as yas_ortalamasi from personel
group by ulke,cinsiyet  -- round  yuvarlama yapar
